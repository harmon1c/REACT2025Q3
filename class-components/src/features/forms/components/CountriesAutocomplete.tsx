...
interface CountryOptionsListProps {
  ...
  activeIndex: number;
  listboxId: string;
  commitValue: (val: string) => void;
  setActiveIndex: (i: number) => void;
}

const CountryOptionsList = memo(function CountryOptionsList({
  filtered,
  activeIndex,
  listboxId,
  commitValue,
  setActiveIndex,
}: CountryOptionsListProps): React.JSX.Element | null {
...
export const CountriesAutocomplete = memo(function CountriesAutocomplete({
  value,
  onChange,
  id,
  name = 'country',
    ...
    <div className={'flex flex-col gap-1 ' + (className || '')}>
      ...
      <div className="relative">
        <input
          ...
          onKeyDown={onKeyDown}
          className="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white/90 dark:bg-gray-800/60 px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-500 motion-safe:transition-colors"
        />
        {open && (
          <CountryOptionsList
            filtered={filtered}
